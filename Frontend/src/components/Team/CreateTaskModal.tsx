...
export const CreateTaskModal: React.FC<CreateTaskModalProps> = ({
  ...
  const [relatedTo, setRelatedTo] = useState<'deal' | 'contact' | 'team' | 'other'>(
    'team',
  );
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Create Task"
      description={`Task for ${memberName}`}
      size="lg"
      footer={modalFooter}
    >
      {/* space-y-5 preserved from the old scroll container, which supplied the
