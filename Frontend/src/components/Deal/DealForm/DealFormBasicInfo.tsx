        ...
        <div>
          ...
          <div className="flex items-center justify-between mt-1.5">
            <div className="flex items-center space-x-3">
              ...
              {/* Format hint — shown only when field is empty and not auto-generating */}
              {!isNameAutoGenerated && !formData.dealName && !hasNameContext && (
                <p className="text-xs text-gray-400">
                  Format: <span className="text-gray-500 font-medium">Company – Product – Month Year</span>
                </p>
              )}
            ...
            {/* Character counter with colour progression */}
            {(() => {
              const len = formData.dealName?.length ?? 0;
              const counterColor =
                len >= 100 ? 'text-red-600 font-semibold' :
                len >= 85  ? 'text-amber-500' :
                len >= 70  ? 'text-yellow-500' :
                             'text-gray-400';
              return (
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className={`text-xs ${counterColor}`}>{len}/100</span>
                </div>
              );
            })()}
          </div>
