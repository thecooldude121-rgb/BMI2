...
/** Mirrors contacts.external_ref VARCHAR(100) (migration 046). */
const MAX_EXTERNAL_REF = 100;
 ...
 *
 * external_ref (migration 046) is deliberately absent. It is provenance a
 * calling module asserts once, at creation, and the whole point of it is that
 * it stays put: Lead Gen's retry recognises its contact by
 * (tenant_id, source, external_ref), so letting a PUT repoint or clear it would
 * allow an ordinary edit to silently detach a contact from the prospect it came
 * from — and the next retry would then create the duplicate the column exists
 * to prevent. createContact sets it explicitly; updateContact ignores it, the
 * same way it ignores any field not on this list.
 */
const WRITABLE = [
...
function validate(body: Record<string, unknown>): string | null {
  ...
  // Checked here rather than left to the column so a caller gets a 400 naming
  // the problem instead of a 22001 string-too-long masked as a 500 by
  // errorHandler. A blank or whitespace-only ref is rejected outright: it would
  // be stored as a value that can never match a later retry, which is worse
  // than sending none, because the caller believes it is protected.
  if (body.external_ref !== undefined && body.external_ref !== null) {
    ...
    if (body.external_ref.trim().length > MAX_EXTERNAL_REF) {
      return `external_ref must be ${MAX_EXTERNAL_REF} characters or fewer`;
    }
  }
  return null;
}
...
export const createContact = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    /**
     * external_ref: the CALLING system's own id for the record it is creating
     * (migration 046). Trimmed, because " 4821" and "4821" are the same
     * prospect to the caller but two different index entries here, and a
     * dedup key that is sensitive to invisible whitespace does not dedup.
     */
    const externalRef = typeof req.body.external_ref === 'string'
      ? req.body.external_ref.trim() || null
      : null;

    // An external_ref is only unique within the system that issued it, so it
    // is meaningless without source naming that system — and the unique index
    // is on (tenant_id, source, external_ref), where a NULL source makes every
    // entry distinct and nothing ever conflicts. Accepting the pair would hand
    // back an external_ref that silently never dedups. Refused at the door with
    // the reason, mirroring contacts_external_ref_source_check, which refuses
    // the same row one layer down for any path that does not come through here.
    if (externalRef && !req.body.source) {
      res.status(400).json({
        success: false,
        message: 'external_ref requires source, which names the system the ref belongs to',
      });
      return;
    }

    // The id is generated by the column DEFAULT (migration 031:
    // 'CT' || LPAD(nextval('contacts_id_seq'), 3, '0')), so it is not computed
    // or sent here. The old MAX(id)+1 read a maximum in one statement and
    // inserted in another, and Node yields between them: two concurrent creates
    // computed the same id and the second collided, which errorHandler masked
    // as a 500. Measured at 5 of 10 lost writes with only two concurrent
    // requests. nextval() is atomic, so the window is gone rather than smaller.
    const cols = [...WRITABLE, 'tenant_id', 'external_ref'];
    const values: any[] = [
      ...WRITABLE.map(c => {
        const v = req.body[c];
        if (v === undefined) {
    ...
    /**
     * IDEMPOTENT ON (tenant_id, source, external_ref) — migration 046.
     *
     * Lead Gen's conversion can succeed here and still lose the response
     * (timeout, restart), after which it retries the same conversion. The
     * conflict target is the partial unique index, so its predicate has to be
     * repeated here; without the WHERE, Postgres cannot infer the index and
     * raises 42P10.
     *
     * DO NOTHING, not DO UPDATE. A retry is the caller asking "did this
     * happen?", not asking to overwrite: the contact may have been edited by a
     * human since the first attempt, and letting a duplicated network packet
     * revert someone's edit would be a worse bug than the one being fixed.
     *
     * When external_ref IS NULL — every contact from the UI, the CSV importer
     * and every caller that predates this — the index predicate is false, the
     * clause can never fire, and this statement behaves exactly as it did
     * before. A duplicate email on those paths still raises 23505 and still
     * comes back as the 409 below.
     *
     * Done in one statement rather than SELECT-then-INSERT for the reason the
     * 23505 catch below already gives: two concurrent retries would both pass a
     * check-then-write. Here the loser conflicts, returns no row, and reads the
     * winner's contact back.
     */
    ...
    if (!inserted.rows[0]) {
      // No row inserted and no error raised means the ON CONFLICT above
      // absorbed it — this exact (source, external_ref) already names a contact
      // in this workspace. Return it, so a retry that lost its response can
      // finally learn the id it created.
      const existing = await pool.query(
        `SELECT ${SELECT_COLUMNS} ${FROM_JOINS}
          WHERE c.tenant_id = $1 AND c.source = $2 AND c.external_ref = $3`,
        [tenantId, req.body.source, externalRef],
      );
      if (existing.rows[0]) {
        // 200, not 201: this request created nothing, and the caller is
        // entitled to know that its earlier attempt was the one that did.
        res.status(200).json({ success: true, data: existing.rows[0] });
        return;
      }
      // Only reachable if the conflicting contact was deleted between the two
      // statements. Nothing was created and nothing was found, so say so
      // rather than return `data: undefined` under `success: true` — a null
      // body behind a 200 is exactly the false confirmation this codebase keeps
      // getting bitten by. A plain retry succeeds.
      res.status(409).json({
        success: false,
        message: 'That contact was deleted while this request ran. Retry.',
      });
      return;
    }
  ...
  try {
    ...
    const summary = await runImport<ContactImportRow>(
      ...
      async (row, _index, tx) => {
        ...
        // ── Id comes from the column DEFAULT (migration 031), like every other
        //    insert path. The old MAX(id)+1 was self-consistent WITHIN one
        //    import — this transaction sees its own uncommitted rows — but not
        //    across requests, and that was the worst instance of the race:
        //    measured against a single concurrent form create, this import
        //    reported created=4 failed=1, so a customer was told one row of
        //    their migration file was bad when the real cause was an id
        //    collision. `RETURNING id` still yields the generated value, which
        //    is all this handler ever used it for.
        const inserted = await tx.query(
          `INSERT INTO contacts (
             tenant_id, first_name, last_name, email, phone, mobile, position,
           ...
           RETURNING id`,
          [
            tenantId, first_name, last_name, email,
