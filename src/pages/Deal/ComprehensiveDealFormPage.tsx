...
export const ComprehensiveDealFormPage: React.FC = () => {
  ...
  const loadDealData = async (dealId: string) => {
    ...
    showToast('Loaded deal: Acme Corp - Enterprise Plan', 'success');
  };
  ...
  const processAccountSelection = (account: any, isHRMS: boolean) => {
    ...
    if (isHRMS) {
      showToast('🏢 HRMS Advantage activated! Win probability boosted to 75%', 'success');
    }
  };
  ...
  const processContactSelection = (contact: any, isHRMS: boolean) => {
    ...
    if (isHRMS) {
      showToast('🏢 HRMS Advantage activated! This is a warm intro opportunity', 'success');
    }
  };
  ...
  const handleApplyAISuggestions = () => {
    if (aiSuggestions) {
      ...
      showToast('✅ Form auto-populated from AI suggestions', 'success');

      // Flash effect on populated fields
      setTimeout(() => {
  ...
  const handleChangeSelection = () => {
    ...
    showToast('Selection cleared, search again', 'info');
  };

  const handleSkipSmartCreation = () => {
    setShowSmartSearch(false);
    showToast('Creating deal from scratch', 'info');
  };
  ...
  const validateField = (field: string, value: any): string | null => {
    switch (field) {
      ...
      case 'dealValue':
        if (!value || value === '' || value === '0') return 'Deal value is required';
        const numValue = parseFloat(value.toString().replace(/,/g, ''));
        if (isNaN(numValue)) return 'Deal value must be a number';
        if (numValue <= 0) return 'Deal value must be greater than $0';
        if (numValue > 10000000) return 'Deal value exceeds maximum ($10,000,000)';
        return null;
  ...
  const calculateWinProbability = (data: any): number => {
    ...
    // Deal value sweet spot boost (40K-60K range)
    const dealValue = parseFloat(data.dealValue?.toString().replace(/,/g, '') || '0');
    if (dealValue >= 40000 && dealValue <= 60000) probability += 5;
  ...
  const handleSave = async (draft: boolean = false) => {
    ...
    setTimeout(() => {
      ...
      showToast(draft ? '💾 Draft saved' : '✅ Deal saved successfully!', 'success');

      // Handle other save options first (before navigation)
      if (!draft) {
        if (saveOptions.createTask) {
          showToast('📋 Opening task creation...', 'info');
          setTimeout(() => {
            showToast('Task creation modal would open here', 'info');
          }, 500);
        }
        if (saveOptions.sendEmail) {
          showToast('📧 Opening email composer...', 'info');
          setTimeout(() => {
            showToast('Email composer would open here', 'info');
          }, 800);
        }
        if (saveOptions.scheduleMeeting) {
          showToast('📅 Opening meeting scheduler...', 'info');
          setTimeout(() => {
            showToast('Meeting scheduler would open here', 'info');
          }, 1100);
        }
      }

      // Execute navigation after actions
      setTimeout(() => {
        if (saveOptions.addAnother) {
          ...
          setValidationErrors({});
          setFieldWarnings({});
          showToast('Ready to add another deal', 'success');
        } else if (saveOptions.viewDeal && !draft) {
          navigate('/crm/deals/1');
        } else if (!draft) {
          navigate('/crm/deals');
        }
      }, saveOptions.createTask || saveOptions.sendEmail || saveOptions.scheduleMeeting ? 1500 : 300);
    }, 1500);
  };
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-6">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-3xl font-bold text-gray-900">
            {isEditMode ? `✏️ Edit Deal: ${formData.dealName}` : '✏️ Add New Deal'}
          </h1>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {duplicateDeals.length > 0 && (
              <DuplicateCheckPanel
                ...
                onMerge={() => showToast('Merge functionality would open', 'info')}
              />
            )}
          </div>
        </div>
